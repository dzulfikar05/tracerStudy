 ...
 public function answerTable($id, Request $request)
{
    ...
    // Ambil questionnaire + pertanyaan
    $questionnaire = Questionnaire::with('questions')->findOrFail($id);
    $questions     = $questionnaire->questions;

    // Mulai query respondents (distinct filler_type + filler_id)
    ...
    // === FILTER-S FILTER DI SINI ===

    if ($request->filled('program_studi')) {
        $respondentsQuery->whereHas('filler_alumni', fn($q) =>
    ...
    if ($request->filled('company')) {
        $respondentsQuery->whereHas('filler_alumni.company', fn($q) =>
    ...
    if ($request->filled('profession_category')) {
        $respondentsQuery->whereHas('filler_alumni.profession', fn($q) =>
    ...
    if ($request->filled('superior')) {
        $respondentsQuery->whereHas('filler_alumni.superior', fn($q) =>
    ...
    // Ambil semua jawaban, group by filler_id
    $answers = Answer::where('questionnaire_id', $id)
        ->whereIn('filler_id', $respondents->pluck('filler_id'))
        ->get()
        ->groupBy('filler_id');

    // Map setiap respondent jadi row dengan jawaban lengkap
    $data = $respondents->map(function ($responden) use ($questions, $answers) {
        ...
        // Data alumni/superior seperti sebelumnya…
        if ($responden->filler_type === 'alumni') {
            $f = $responden->filler_alumni;
            $row = array_merge($row, [
                'study_program'        => $f->study_program,
                'nim'                  => $f->nim,
                'full_name'            => $f->full_name,
                'phone'                => $f->phone,
                'email'                => $f->email,
                'study_start_year'     => $f->study_start_year,
                'graduation_date'      => $f->graduation_date,
                'graduation_year'      => date('Y', strtotime($f->graduation_date)),
                'start_work_date'      => $f->start_work_date,
                'waiting_time'         => $f->waiting_time,
                'start_work_now_date'  => $f->start_work_now_date,
                'company_type'         => $f->company?->company_type,
                'company_name'         => $f->company?->name,
                'company_scope'        => $f->company?->scope,
                'company_address'      => $f->company?->address,
                'profession_category'  => $f->profession?->profession_category?->name,
                'profession'           => $f->profession?->name,
                'superior_name'        => $f->superior?->full_name,
                'superior_position'    => $f->superior?->position,
                'superior_phone'       => $f->superior?->phone,
                'superior_email'       => $f->superior?->email,
            ]);
        } else {
            // mapping untuk filler_type = 'superior'…
            // (sama seperti kode lama-mu)
        }

        // Isi jawaban untuk setiap pertanyaan
        foreach ($questions as $q) {
            $row["q_{$q->id}"] = $responseAnswers->firstWhere('question_id', $q->id)->answer ?? '-';
        }
    ...
    return DataTables::of($data)
        ->addIndexColumn()
            ->addColumn('action', function($row) {
        // Ganti URL/data-attributes sesuai kebutuhan
        return '
            <div class="dropdown">
